...
INSERT INTO public.matches (home_team, away_team, match_time, status, stage) VALUES
  ...
  ('Winner A', 'Runner-up B', now() + interval '15 days', 'upcoming', 'R16'),
  -- R16 Match 2: Winner C vs Runner-up D
  ('Winner C', 'Runner-up D', now() + interval '15 days 3 hours', 'upcoming', 'R16'),
  -- R16 Match 3: Winner E vs Runner-up F
  ('Winner E', 'Runner-up F', now() + interval '15 days 6 hours', 'upcoming', 'R16'),
  -- R16 Match 4: Winner G vs Runner-up H
  ('Winner G', 'Runner-up H', now() + interval '15 days 9 hours', 'upcoming', 'R16'),
  -- R16 Match 5: Runner-up A vs Winner B
  ('Runner-up A', 'Winner B', now() + interval '16 days', 'upcoming', 'R16'),
  -- R16 Match 6: Runner-up C vs Winner D
  ('Runner-up C', 'Winner D', now() + interval '16 days 3 hours', 'upcoming', 'R16'),
  -- R16 Match 7: Runner-up E vs Winner F
  ('Runner-up E', 'Winner F', now() + interval '16 days 6 hours', 'upcoming', 'R16'),
  -- R16 Match 8: Runner-up G vs Winner H
  ('Runner-up G', 'Winner H', now() + interval '16 days 9 hours', 'upcoming', 'R16');

-- Insert Quarter Final matches
INSERT INTO public.matches (home_team, away_team, match_time, status, stage) VALUES
  -- QF Match 1: Winner R16-1 vs Winner R16-2
  ('Winner R16-1', 'Winner R16-2', now() + interval '20 days', 'upcoming', 'QF'),
  -- QF Match 2: Winner R16-5 vs Winner R16-6
  ('Winner R16-5', 'Winner R16-6', now() + interval '20 days 4 hours', 'upcoming', 'QF'),
  -- QF Match 3: Winner R16-3 vs Winner R16-4
  ('Winner R16-3', 'Winner R16-4', now() + interval '21 days', 'upcoming', 'QF'),
  -- QF Match 4: Winner R16-7 vs Winner R16-8
  ('Winner R16-7', 'Winner R16-8', now() + interval '21 days 4 hours', 'upcoming', 'QF');

-- Insert Semi Final matches
INSERT INTO public.matches (home_team, away_team, match_time, status, stage) VALUES
  -- SF Match 1: Winner QF-1 vs Winner QF-2
  ('Winner QF-1', 'Winner QF-2', now() + interval '25 days', 'upcoming', 'SF'),
  -- SF Match 2: Winner QF-3 vs Winner QF-4
  ('Winner QF-3', 'Winner QF-4', now() + interval '26 days', 'upcoming', 'SF');

-- Insert 3rd Place match
INSERT INTO public.matches (home_team, away_team, match_time, status, stage) VALUES
  ('Loser SF-1', 'Loser SF-2', now() + interval '29 days', 'upcoming', '3P');

-- Insert Final match
INSERT INTO public.matches (home_team, away_team, match_time, status, stage) VALUES
  ('Winner SF-1', 'Winner SF-2', now() + interval '30 days', 'upcoming', 'Final');
